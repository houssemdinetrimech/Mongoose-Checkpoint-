...
const createAndSavePerson = (req,res) => {
    ...
    let houssem = new Person ({name:'houssem', age : 30, favoriteFoods: ['pasta'] });

    houssem.save(req.body) 
...
let  arrayOfPeople = [
    ...
    {name: 'sami', age:20, favoriteFoods: ['burger','pizza', 'burritos']},
];
...
const personName = (req,res) => {
    Person.findOneAndUpdate ({name:'houssem'}, {age:20})
    .then(response => res.json(response))
    .catch(err => res.status(500).json(err))
}
